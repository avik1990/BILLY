...
public class DatabaseHelperPRESENTMETERUPDATE extends SQLiteOpenHelper {
    ...
    public DatabaseHelperPRESENTMETERUPDATE(Context context) {
    ...
    public List<NSBMData> getNBSMRInstallationOnly(String flag) {
        ...
        if (flag.equalsIgnoreCase("2")) {
           // selectQuery = "SELECT INSTALLATION,MRREASON FROM TBL_SPOTBILL_HEADER_DETAILS";
            selectQuery = "SELECT INSTALLATION,PRESENT_READING_REMARK FROM TBL_SPOTBILL_HEADER_DETAILS";
        } else {
            //selectQuery = "SELECT INSTALLATION,MRREASON FROM TBL_SPOTBILL_HEADER_DETAILS where READ_FLAG='1' AND SENT_FLAG='0'";
            selectQuery = "SELECT INSTALLATION,PRESENT_READING_REMARK FROM TBL_SPOTBILL_HEADER_DETAILS where READ_FLAG='1' AND SENT_FLAG='0'";
        }
        ...
        if (cursor.moveToFirst()) {
            do {
                nSBMDataDatamodel = new NSBMData();
                nSBMDataDatamodel.setINSTALLATION(cursor.getString(cursor.getColumnIndex("INSTALLATION")));
                nSBMDataDatamodel.setMRREASON(cursor.getString(cursor.getColumnIndex("PRESENT_READING_REMARK")));
    ...
    public String getInstallatioationNo(String param, int offset) {
        ...
        if (offset >= 0) {
            selectQuery = "select INSTALLATION from TBL_SPOTBILL_HEADER_DETAILS where READ_FLAG ='0' and USER_TYPE='" + USER_TYPE + "'  order by SEQ ASC LIMIT 1 OFFSET " + offset + "";
        } else {
    ...
    public void updateNMRemarksNonSbmReading(MeterOkNonSbmReadingModel meterOkNonSbmReadingModel, String installationNo) {
        ...
        String sqlquery =
                "UPDATE TBL_SPOTBILL_HEADER_DETAILS SET " +
                        ...
                        "PRESENT_READING_REMARK='" + meterOkNonSbmReadingModel.getMr_reason() + "'," +
            ...
            try {
                ...
                for (int i = 0; i < response.response.size(); i++) {
                    ...
                    for (int j = 0; j < headrelist.size(); j++) {
                        ...
                        values.put("SENDER_MOBILE", PreferenceHandler.getisUserId(mContext));
                       /* try {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (rowInserted > 0) {
                ...
                /*new CustomDialogWithTwoButton(mContext, "Download Status",
                        ...
                        }, String.valueOf(totalCount), String.valueOf(Headercount), String.valueOf(ChildCount)).show();*/
            } else {
...
*/
        ...
        String query = "select LEGACY_ACCOUNT_NO,NAME,ADDRESS1,ADDRESS2,SEQ,SUB_SEQ,LEGACY_ACCOUNT_NO2,RATE_CATEGORY,DIV,SUB_DIV,SECTION,CONSUMER_OWNED," +
                ...
                "case when instr(ifnull(MISC_CHARGES,0),'-') > 0 then (substr(ifnull(MISC_CHARGES,0),1,length(ifnull(MISC_CHARGES,0))-1)+0)*-1 else ifnull(MISC_CHARGES,0)+0 end MISC_CHARGES," +
                "case when instr(ifnull(CR_ADJ,0),'-') > 0 then (substr(ifnull(CR_ADJ,0),1,length(ifnull(CR_ADJ,0))-1)+0)*-1 else ifnull(CR_ADJ,0)+0 end CR_ADJ," +
                "case when instr(ifnull(DB_ADJ,0),'-') > 0 then (substr(ifnull(DB_ADJ,0),1,length(ifnull(DB_ADJ,0))-1)+0)*-1 else ifnull(DB_ADJ,0)+0 end DB_ADJ," +
                "PRV_BILLED_AMT," +
                "case when instr(ifnull(PREVIOUS_BILLED_PROV_UNIT,0),'-') > 0 then (substr(ifnull(PREVIOUS_BILLED_PROV_UNIT,0),1,length(ifnull(PREVIOUS_BILLED_PROV_UNIT,0))-1)+0)*-1 else ifnull(PREVIOUS_BILLED_PROV_UNIT,0)+0 end PREVIOUS_BILLED_PROV_UNIT," +
                "LAST_PAID_DATE,LAST_PYMT_RCPT," +
                "case when instr(ifnull(LAST_PAID_AMT,0),'-') > 0 then (substr(ifnull(LAST_PAID_AMT,0),1,length(ifnull(LAST_PAID_AMT,0))-1)+0)*-1 else ifnull(LAST_PAID_AMT,0)+0 end LAST_PAID_AMT," +
                "ED_EXEMPT,AIFI,NEW_METER_NO,SDI,ASD,ASDAA,INSTALLATION,SBM_BILL_NO,CA,PRV_ARR," +
                "case when instr(ifnull(ARREARS,0),'-') > 0 then (substr(ifnull(ARREARS,0),1,length(ifnull(ARREARS,0))-1)+0)*-1 else ifnull(ARREARS,0)+0 end ARREARS," +
                "case when instr(ifnull(ULF,0),'-') > 0 then (substr(ifnull(ULF,0),1,length(ifnull(ULF,0))-1)+0)*-1 else ifnull(ULF,0)+0 end ULF," +
                "case when instr(ifnull(PREV_BILL_UNITS,0),'-') > 0 then (substr(ifnull(PREV_BILL_UNITS,0),1,length(ifnull(PREV_BILL_UNITS,0))-1)+0)*-1 else ifnull(PREV_BILL_UNITS,0)+0 end PREV_BILL_UNITS," +
                "BILL_MONTH," +
                "case when instr(ifnull(ECS_LIMT,0),'-') > 0 then (substr(ifnull(ECS_LIMT,0),1,length(ifnull(ECS_LIMT,0))-1)+0)*-1 else ifnull(ECS_LIMT,0)+0 end ECS_LIMT," +
                "ECS_VALIDITY_PERIOD,PRESENT_READING_REMARK,PRESENT_METER_STATUS," +
                "case when instr(ifnull(PRESENT_BILL_UNITS,0),'-') > 0 then (substr(ifnull(PRESENT_BILL_UNITS,0),1,length(ifnull(PRESENT_BILL_UNITS,0))-1)+0)*-1 else ifnull(PRESENT_BILL_UNITS,0)+0 end PRESENT_BILL_UNITS," +
                "PRESENT_BILL_TYPE," +
                "case when instr(ifnull(EC,0),'-') > 0 then (substr(ifnull(EC,0),1,length(ifnull(EC,0))-1)+0)*-1 else ifnull(EC,0)+0 end EC," +
                "case when instr(ifnull(MMFC,0),'-') > 0 then (substr(ifnull(MMFC,0),1,length(ifnull(MMFC,0))-1)+0)*-1 else ifnull(MMFC,0)+0 end MMFC," +
                "case when instr(ifnull(MRENT_CHARGED,0),'-') > 0 then (substr(ifnull(MRENT_CHARGED,0),1,length(ifnull(MRENT_CHARGED,0))-1)+0)*-1 else ifnull(MRENT_CHARGED,0)+0 end MRENT_CHARGED," +
                "case when instr(ifnull(ED,0),'-') > 0 then (substr(ifnull(ED,0),1,length(ifnull(ED,0))-1)+0)*-1 else ifnull(ED,0)+0 end ED," +
                ...
                "case when instr(ifnull(AVERAGE_KWH,0),'-') > 0 then (substr(ifnull(AVERAGE_KWH,0),1,length(ifnull(AVERAGE_KWH,0))-1)+0)*-1 else ifnull(AVERAGE_KWH,0)+0 end AVERAGE_KWH," +
                "BILL_BASIS,BILL_NO,INVOICE_NO,READ_ONLY,BP_START_DATE,BP_END_DATE,PPAC,ROUND_OFF," +
                "case when instr(ifnull(SEC_DEPOSIT_AMT,0),'-') > 0 then (substr(ifnull(SEC_DEPOSIT_AMT,0),1,length(ifnull(SEC_DEPOSIT_AMT,0))-1)+0)*-1 else ifnull(SEC_DEPOSIT_AMT,0)+0 end SEC_DEPOSIT_AMT," +
                ...
                "case when instr(ifnull(PPI,0),'-') > 0 then (substr(ifnull(PPI,0),1,length(ifnull(PPI,0))-1)+0)*-1 else ifnull(PPI,0)+0 end PPI," +
                ...
                "case when instr(ifnull(DPS_BILLED,0),'-') > 0 then (substr(ifnull(DPS_BILLED,0),1,length(ifnull(DPS_BILLED,0))-1)+0)*-1 else ifnull(DPS_BILLED,0)+0 end DPS_BILLED," +
    ...
    public Cursor getChilddetails(String installcommaseperated) {
        ...
        if (installcommaseperated.isEmpty()) {
            query = "select * from TBL_SPOTBILL_CHILD_DETAILS WHERE USER_TYPE='" + USER_TYPE + "'";
        } else {
            query = "select * from TBL_SPOTBILL_CHILD_DETAILS WHERE INSTALLATION in " + installcommaseperated + " AND USER_TYPE='" + USER_TYPE + "'";
        }
    ...
    public Cursor getUserChilddetails(String condition) {
        ...
        String query = "SELECT * FROM (select * from(select distinct ABLBELNR,PREV_READ_DATE,SCHEDULE_METER_READ_DATE," +
                ...
                " PRS_MD, REGISTER_CODE,  NO_OF_REG, TRANS_TYPE from TBL_SPOTBILL_CHILD_DETAILS " + condition + "))" + "GROUP BY INSTALLATION,REGISTER_CODE";
    ...
    public void deleteDataHeader1() {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("DELETE FROM TBL_SPOTBILL_HEADER_DETAILS");
        db.close();
    }

    public void deleteDataChild1() {
        SQLiteDatabase db = this.getWritableDatabase();
